        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 24) {
                ...
                HStack {
                    ...
                    Button(model.t("translate.clipboard")) {
                        controller.translateClipboard()
                    }
                    .buttonStyle(.plain)
                    ...
                    ScrollView {
                        VStack(alignment: .leading, spacing: 20) {
                            VStack(alignment: .leading, spacing: 6) {
                                ...
                                Text(record.inputOrigin.localizedName(in: model.preferences.interfaceLanguage))
                                    .font(.system(size: 11, weight: .bold))
                                    .padding(.horizontal, 10)
                                    .padding(.vertical, 6)
                                    .background(Color.langxSurface)
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 24) {
                ...
                VStack(alignment: .leading, spacing: 18) {
                    SettingsSectionTitle(icon: "keyboard", title: model.t("settings.integrations"))

                    VStack(alignment: .leading, spacing: 18) {
                        VStack(alignment: .leading, spacing: 14) {
                            Text(model.t("settings.quick_shortcut"))
                                .font(.system(size: 14, weight: .semibold))

                            HStack(spacing: 12) {
                                ForEach(QuickActionShortcutPreset.allCases) { preset in
                                    SelectionChip(
                                        title: preset.localizedName(in: model.preferences.interfaceLanguage),
                                        isSelected: model.preferences.quickActionShortcut == preset
                                    ) {
                                        model.setQuickActionShortcut(preset)
                                    }
                                }
                            }
                        }
                        ...
                        Toggle(isOn: Binding(
                            get: { model.preferences.populateClipboardOnQuickOpen },
                            set: { model.setPopulateClipboardOnQuickOpen($0) }
                        )) {
                            VStack(alignment: .leading, spacing: 4) {
                                Text(model.t("settings.clipboard_prefill"))
                                    .font(.system(size: 14, weight: .semibold))
                                Text(model.t("settings.clipboard_prefill_hint"))
                                    .font(.system(size: 12))
                                    .foregroundStyle(Color.langxSubtext)
                            }
                        }
                        ...
                        Toggle(isOn: Binding(
                            get: { model.preferences.autoCopyTranslationResult },
                            set: { model.setAutoCopyTranslationResult($0) }
                        )) {
                            VStack(alignment: .leading, spacing: 4) {
                                Text(model.t("settings.auto_copy"))
                                    .font(.system(size: 14, weight: .semibold))
                                Text(model.t("settings.auto_copy_hint"))
                                    .font(.system(size: 12))
                                    .foregroundStyle(Color.langxSubtext)
                            }
                        }
                        ...
                        Toggle(isOn: Binding(
                            ...
                            VStack(alignment: .leading, spacing: 4) {
                                Text(model.t("settings.menu_bar"))
                                    .font(.system(size: 14, weight: .semibold))
                                Text(model.t("settings.menu_bar_hint"))
                                    .font(.system(size: 12))
                                    .foregroundStyle(Color.langxSubtext)
                            }
                        }
...
struct QuickTranslatePanelRootView: View {
    @EnvironmentObject private var model: AppModel
    @EnvironmentObject private var controller: AppController
    ...
    var body: some View {
        ZStack {
            LinearGradient(
                colors: [Color.langxBackground, Color.white],
            ...
            VStack(alignment: .leading, spacing: 20) {
                HStack(alignment: .top) {
                    VStack(alignment: .leading, spacing: 6) {
                        Text(model.t("quick.title"))
                            .font(.system(size: 24, weight: .bold, design: .rounded))
                        Text(model.t("quick.subtitle"))
                            .font(.system(size: 13))
                            .foregroundStyle(Color.langxSubtext)
                    }
                    ...
                    Button(model.t("quick.close")) {
                        controller.closeQuickTranslatePanel()
                    }
                    .buttonStyle(.plain)
                    .padding(.horizontal, 14)
                    .padding(.vertical, 10)
                ...
                HStack(spacing: 10) {
                    Pill(text: model.detectedSourceLanguage.localizedName(in: model.preferences.interfaceLanguage), tone: .neutral)
                    Pill(text: model.preferences.selectedEngine.displayName, tone: .primary)
                    Pill(text: model.preferences.defaultTargetLanguage.localizedName(in: model.preferences.interfaceLanguage), tone: .warm)
                }
                ...
                HStack(spacing: 12) {
                    Button(model.isTranslating ? model.t("translate.translating") : model.t("translate.cta")) {
                        model.translate()
                    }
                    .buttonStyle(.plain)
                ...
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    ScrollView {
                        Text(model.translatedText.isEmpty ? model.t("translate.placeholder") : model.translatedText)
                            .frame(maxWidth: .infinity, alignment: .leading)
                            .font(.system(size: 16))
                            .foregroundStyle(model.translatedText.isEmpty ? Color.langxSubtext.opacity(0.7) : Color.langxText)
                            .textSelection(.enabled)
                    }
                    .frame(minHeight: 170)

                    Text(model.errorMessage ?? model.t("quick.footer"))
                        .font(.system(size: 12, weight: .medium))
                        .foregroundStyle(model.errorMessage == nil ? Color.langxSubtext : .red)
                }
                .langxCardStyle()
            }
            .padding(24)
        ...
        .onChange(of: model.quickTranslateActivationToken) { _, _ in
            focusSourceEditor()
        }
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text(model.t("menu.subtitle"))
                    .font(.system(size: 12, weight: .medium))
                    .foregroundStyle(Color.langxSubtext)
            }

            Button(model.t("menu.open_quick")) {
                controller.showQuickTranslatePanel(prefillFromClipboard: model.preferences.populateClipboardOnQuickOpen)
            }
            .menuBarActionStyle(prominent: true)

            Button(model.t("menu.translate_clipboard")) {
                controller.translateClipboard()
            }
            .menuBarActionStyle()

            Button(model.t("menu.copy_latest")) {
                model.copyLatestTranslation()
            }
            .menuBarActionStyle()
            ...
            VStack(alignment: .leading, spacing: 10) {
                Text(model.t("menu.recent"))
                    .font(.system(size: 11, weight: .bold))
                    .tracking(1.2)
                    .foregroundStyle(Color.langxSubtext)

                if model.recentHistory.isEmpty {
                    Text(model.t("menu.recent_empty"))
                        .font(.system(size: 12))
                        .foregroundStyle(Color.langxSubtext)
                } else {
                    ForEach(model.recentHistory) { record in
                        Button {
                            controller.loadRecordInQuickTranslate(record)
                        } label: {
                            VStack(alignment: .leading, spacing: 4) {
                                Text(record.sourceText)
                                    .lineLimit(2)
...
private extension View {
    func menuBarActionStyle(prominent: Bool = false) -> some View {
        self
            .buttonStyle(.plain)
            .frame(maxWidth: .infinity, alignment: .leading)
            .padding(.horizontal, 14)
            .padding(.vertical, 12)
            .background(prominent ? AnyShapeStyle(LinearGradient.langxPrimary) : AnyShapeStyle(Color.langxSurface))
            .foregroundStyle(prominent ? AnyShapeStyle(Color.white) : AnyShapeStyle(Color.langxText))
            .clipShape(RoundedRectangle(cornerRadius: 14, style: .continuous))
    }
}
